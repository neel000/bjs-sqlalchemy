...
class IntregrationTesting(TestClient):
    ...
    async def crud(self):
        ...
        status, update_data = await data.save(session=session)
    ...
    def test_query_for_crud(self):
        asyncio.run(self.crud())
...
class TestCharFieldModelperation(TestClient, Common):
    ...
    async def blank_data_create(self):
        session = DatabaseConfig()
        obj = self.model()
        status, data = await obj.save(session=session)
    ...
    def test_blank_data_create(self):
        asyncio.run(self.blank_data_create())

    async def create_valid_data(self):
        session = DatabaseConfig()
    ...
    def test_create_valid_data(self):
        asyncio.run(self.create_valid_data())

    async def null_check_no_data(self):
        session = DatabaseConfig()
        obj = self.null_model()
        status, data = await obj.save(session=session)
        assert status
    ...
    def test_null_check_no_data(self):
        asyncio.run(self.null_check_no_data())
    
    async def null_check_blank_data(self):
        session = DatabaseConfig()
    ...
    def test_null_check_blank_data(self):
        asyncio.run(self.null_check_blank_data())
